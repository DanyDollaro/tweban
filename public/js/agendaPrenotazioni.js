...
function showAcceptModal(id) {
    ...
    $('#accept-giorno').val(formattedDate);
    $('#accept-orario').val(formattedTime);
...
function confirmAcceptReservation() {
    ...
    $.ajax({
        url: `/staff/prenotazioni/${id}/accetta`,
        method: 'POST',
        data: {
            _token: $('meta[name="csrf-token"]').attr('content'),
            data_prenotazione: giorno,
        ...
        success: function (response) {
            ...
            row.find('td:nth-child(1)').text(giorno);
            row.find('td:nth-child(2)').text(orario);
